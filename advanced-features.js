...
window.analyzeCompetitorAsset = async function () {
    ...
    // Platform, Asset ID, এবং URL Slug ডিটেকশন
    let platform = "Adobe Stock";
    let assetId = urlInput;
    let urlSlug = ""; // URL থেকে descriptive words extract

    if (urlInput.toLowerCase().includes('shutterstock.com')) {
        platform = "Shutterstock";
        // Shutterstock URL: /image-photo/woman-working-laptop-123456789
        const match = urlInput.match(/(?:image(?:-photo|-vector|-illustration)?\/)(.+?)-(\d+)/) || urlInput.match(/-(\d+)\/?$/);
        if (match) {
            if (match[2]) {
                assetId = match[2];
                urlSlug = match[1].replace(/-/g, ' ').trim(); // "woman-working-laptop" → "woman working laptop"
            } else {
                assetId = match[1];
            }
        }
        ...
        // Adobe Stock URL formats:
        // /images/beautiful-sunset-over-ocean/2067924530
        // /in/stock-photo/id/2067924530?asset_id=2067924530
        // /2067924530
        const idMatch = urlInput.match(/(\d{7,12})/);
        if (idMatch) assetId = idMatch[1];

        // Extract slug from URL path (descriptive words between /images/ or similar and /id)
        const slugMatch = urlInput.match(/\/images\/([^\/]+)\/\d/) || urlInput.match(/\/stock-photo\/([^\/]+)\/\d/);
        if (slugMatch && slugMatch[1] !== 'id') {
            urlSlug = slugMatch[1].replace(/-/g, ' ').trim();
        }
    } else if (/^\d{7,12}$/.test(urlInput)) {
        // শুধু numeric ID দেওয়া হলে
        platform = "Adobe Stock";
        assetId = urlInput;
    }
